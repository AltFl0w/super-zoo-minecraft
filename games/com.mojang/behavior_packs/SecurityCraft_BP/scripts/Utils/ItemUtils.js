...
class ItemUtils {
    ...
    reinforceBlock() {
        ...
        if (item?.typeId !== 'scb:reinforcer' || block.typeId.includes('scb:'))
            return;
    ...
    placeTurret() {
        ...
        if (item?.typeId !== 'scb:turret_item')
            return;
        this.setItemStack();
    ...
    turretModule() {
        ...
        if (item?.typeId !== 'scb:turret_module')
            return;
    ...
    mineModule() {
        const { item, player } = this;
        if (item?.typeId !== 'scb:mine_module')
            return;
        const itemForms = new form(player, item, undefined, undefined);
    ...
    cameraModule() {
        const { item, player } = this;
        if (item?.typeId !== 'scb:tablet')
            return;
        const itemForms = new form(player, item, undefined, undefined);
        itemForms.cameraModule();
    }
    // Place scb thing tamed
    setTamed(entityId, player) {
        const { block } = this;
        const { location, dimension } = block;
        const pos = {
            x: location.x + 0.5,
    ...
    transferContainer() {
        ...
        if (!chest?.typeId.includes('reinforced_chest'))
            return;
        if (item?.typeId !== 'scb:universal_chest_transferer')
            return;
    ...
    universalRemover() {
        ...
        const item = inv.getItem(player.selectedSlotIndex);
        if (item?.typeId !== 'scb:universal_block_remover')
            return;
        db.set(`${location.x} ${location.y} ${location.z}:password`, false);
        db.set(`${location.x} ${location.y} ${location.z}:locked`, false);
