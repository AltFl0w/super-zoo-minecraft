...
class BlockUtils {
    ...
    setMine() {
        ...
        if (block.typeId !== 'scb:mine')
            return;
        const itemForms = new form(player, undefined, undefined, block);
        itemForms.setName();
    }
    // Camera Config
    setCamera() {
        const { player, block } = this;
        if (block.typeId !== 'scb:security_camera')
            return;
        const itemForms = new form(player, undefined, undefined, block);
        itemForms.setName();
    }
    ...
    setReinforcedChest() {
        ...
        if (!block.typeId.includes('scb:reinforced_chest'))
            return;
        if (block.typeId === 'scb:reinforced_chest') {
            ...
            chest.getComponent('tameable').tame(player);
            chest.nameTag = player.name + "'s Chest";
        }
        else {
    ...
    setScanner() {
        const { player, block } = this;
        const { dimension, location: loc } = block;
        if (block.typeId !== ('scb:scanner'))
            return;
        const type = block.typeId.split(':')[1];
        db.setOwner(block.location, player, type);
    }
    // Set keypad reinforced
    setKeypad() {
        const { player, block } = this;
        const { dimension, location: loc } = block;
        if (block.typeId !== ('scb:keypad'))
            return;
        const type = block.typeId.split(':')[1];
        db.setOwner(block.location, player, type);
    ...
    setReinforcedDoor() {
        ...
        db.setOwner(loc, player, type);
    }
    // Trophy System
    setTrophySystem() {
        const { player, block } = this;
        const { dimension, location: loc } = block;
        if (block.typeId !== 'scb:trophy_system')
            return;
        const type = block.typeId.split(':')[1];
    ...
    setKeycardReader() {
        const { player, block } = this;
        const { dimension, location: loc } = block;
        if (block.typeId !== 'scb:keycard_reader')
            return;
        const type = block.typeId.split(':')[1];
        db.setOwner(block.location, player, type);
    ...
    getNewCard() {
        ...
        if (this.item?.typeId === 'scb:keycard') {
            ...
            if (!isOwner)
                return;
            this.item?.setLore([`Owner: ${player.name}`]);
            this.item?.setDynamicProperty('scb:owner', player.id);
            this.inv.setItem(player.selectedSlotIndex, this.item);
        }
    }
    cardAccess() {
        const { player, block } = this;
        const { dimension, location: loc } = block;
        const active = db.get(`${loc.x} ${loc.y} ${loc.z}:reader_active`);
        if (active)
            return;
        if (block.typeId !== 'scb:keycard_reader')
            return;
        if (this.item?.typeId === 'scb:keycard') {
            ...
            if (!hasLore)
                return;
            const owner = this.item.getDynamicProperty('scb:owner');
    ...
    removePiston() {
        ...
        if (!this.item?.typeId.includes("piston"))
            return;
        if (!pistonsConfig.active)
            return;
            ...
            for (let y = -distance; y <= distance; y++) {
                for (let z = -distance; z <= distance; z++) {
                    ...
                    const reinforced = world
                        ...
                        .find((e) => e.includes(`${currentLocation.x} ${currentLocation.y} ${currentLocation.z}`));
                    if (reinforced === "" || reinforced === undefined)
                        continue;
                    scb.cancel = true;
                }
            }
    ...
    waila() {
        ...
        if (item || item?.typeId === 'scb:remover')
            return;
        if (info) {
