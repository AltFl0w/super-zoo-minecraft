import { world, system, Entity, Player, Block, ItemStack, Vector3, Dimension, System, EasingType } from "@minecraft/server";
...
class ItemFormUtils {
    ...
    constructor(player: Player, item: ItemStack | undefined, entity: Entity | undefined, block: Block | undefined) {
        this.player = player;
        this.item = item;
        this.entity = entity;
        this.block = block;
    ...
    private spawnStack(dimension: Dimension, location: Vector3, type: string) {
        const stack = new ItemStack(type);
        dimension.spawnItem(stack, location);
    }

    private getType(player: Player, type: string) {
        try {
            ...
            switch (scb.selection) {
                case 0: {
                    ...
                    modal.show(player).then(scb => {
                        if (scb.canceled) return;
                        const mine = getType?.types[scb.formValues[0] as number] as string;
    ...
    setName() {
        ...
        form.show(player).then(scb => {
            if (scb.canceled) {
                ...
                const scbName = scb.formValues[0] as string;
