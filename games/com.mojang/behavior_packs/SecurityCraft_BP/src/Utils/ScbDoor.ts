import { world, system, BlockCustomComponent, Block, MinecraftDimensionTypes, ItemStack, Vector3, Dimension, BlockPermutation, Player } from '@minecraft/server'
import DataBase from './DataBase'

const db = DataBase;

function dimensionToHeight(dimension: string | MinecraftDimensionTypes) {
    const heights = [
        {
...
//the blockcomponent interface
interface blockComp {
    //the id of the block component
    id: string,
    //the code of the block component
    code: BlockCustomComponent
}

const blockComps: blockComp[] = [
    //define the block component
    {
        ...
        code: {
            onTick: (data) => {
                ...
                } else try { doorPart = block.below(1) } catch { } //gets the block below
...
class doorManager {
    //set the door tag
    static doorTag = "scb:is_door"
    //interact with a door block
    static removeDoor(block: Block, player: Player) {
        const dim = block.dimension;
        const loc = block.location;
        const inv = player.getComponent('inventory').container;
        const itemStack = inv.getItem(player.selectedSlotIndex);
        if (itemStack?.typeId !== 'scb:remover') return;
    ...
    static interactWithDoor(block: Block) {
        ...
        if (doorPart != undefined) {
            ...
            if (open == true) {
                ...
                if (data != undefined && data.closeSound != undefined) dim.playSound(data.closeSound.id, loc, { pitch: data.closeSound.pitch, volume: data.closeSound.volume })
                bool = false
            } else {
                //play the open sound and set the bool constant to true
                if (data != undefined && data.openSound != undefined) dim.playSound(data.openSound.id, loc, { pitch: data.openSound.pitch, volume: data.openSound.volume })
                bool = true
            }
    ...
    static breakDoor(blockID: string, block: Block, topHalf: boolean) {
        //does this stuff a tick later
        system.runTimeout(() => {
            let doorPart: Block = undefined
            if (topHalf == false) {
                try { doorPart = block.above(1) } catch { }
            } else try { doorPart = block.below(1) } catch { }
        ...
        system.runTimeout(() => {
            ...
            aboveBlock.setPermutation(
                aboveBlock.permutation
                    ...
                    .withState("scb:reversed", reversed)
            ); //sets the facing direction to the main blocks facing direction
        });
    }
    static doors: door[] = [
        //door data
