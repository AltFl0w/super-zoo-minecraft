import { Player, system, world, Block, Vector3 } from "@minecraft/server";
...
world.beforeEvents.worldInitialize.subscribe(scb => {
    ...
    scb.itemComponentRegistry.registerCustomComponent('scb:item', {
        onUseOn({ itemStack: item, source: player, block }) {
            const items = new ItemUtils(item, player as Player, block);
        ...
        onUse({ itemStack: item, source: player }) {
            const items = new ItemUtils(item, player as Player, undefined);
            items.turretModule();
            items.mineModule();
            items.cameraModule();
        }
