...
class SuperZooCaretaker {
    ...
    loadConfig() {
        try {
            ...
            this.restrictedCommands = new Set(config.restrictedCommands || [
                'emergency', 'clean', 'schedule', 'authorize'
            ]);
            
            // Store full config for other uses
            this.config = config;
    ...
    showHelp(player) {
        ...
        const staffCommands = [
            ...
            '§e!tnt-auth <add/remove> <player> §7- Manage TNT permissions',
            '§e!tnt-list §7- List users authorized for TNT/explosives'
        ];
    ...
    listTNTUsers(requestingPlayer) {
        // Only authorized users can view TNT permissions
        if (!this.authorizedUsers.has(requestingPlayer)) {
            this.sendMessage(`§c🔒 Access denied! Only authorized staff can view TNT permissions.`);
            return;
        }

        this.sendMessage('§6🧨 TNT Security Information:');
        this.sendMessage('§e📋 TNT permissions are managed by the TNT Security behavior pack');
        this.sendMessage('§e💡 Use in-game commands: !tnt-auth <add/remove> <player>');
        this.sendMessage('§e📊 Current zoo staff with potential TNT access:');
        
        Array.from(this.authorizedUsers).forEach(user => {
            this.sendMessage(`§a- ${user}`);
        });
        
        this.sendMessage('§c⚠️ Note: TNT access must be granted separately from zoo staff permissions');
    }
